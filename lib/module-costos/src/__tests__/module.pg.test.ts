...
suite("DGP-021.1 · Costos · PostgreSQL", { timeout: 30_000 }, () => {
  ...
  it("precisión numeric(18,6) EXACTA dígito a dígito (sin float)", async () => {
    const h = must(await exec(ctx(T_A), `${MODULO}.hecho.materializar-material`, {
      opId: `p-${RUN}`, otId: "ot1", articuloId: "art1", cantidad: "3.000000", unidad: "UN", moneda: "COP",
  ...
  it("doble materialización CONCURRENTE con mismo opId ⇒ un solo hecho (índice único (tenant, op_id))", async () => {
    ...
    const [a, b] = await Promise.all([
      exec(ctx(T_A), `${MODULO}.hecho.materializar-material`, { opId: op, otId: "ot1", articuloId: "art1", cantidad: "1", unidad: "UN", moneda: "COP" }),
      exec(ctx(T_A), `${MODULO}.hecho.materializar-material`, { opId: op, otId: "ot1", articuloId: "art1", cantidad: "1", unidad: "UN", moneda: "COP" }),
    ]);
  ...
  it("SNAPSHOT inmutable a nivel de BD: cambiar el costo origen no altera la fila persistida", async () => {
    const op = `snap-${RUN}`;
    const h = must(await exec(ctx(T_A), `${MODULO}.hecho.materializar-material`, {
      opId: op, otId: "ot1", articuloId: "art1", cantidad: "1.000000", unidad: "UN", moneda: "COP",
    })) as Record<string, unknown>;
  ...
  it("aislamiento cross-tenant (RLS): los hechos de T_A no se ven desde T_B", async () => {
    must(await exec(ctx(T_B), `${MODULO}.hecho.materializar-material`, { opId: `iso-${RUN}`, otId: "ot1", articuloId: "art1", cantidad: "1", unidad: "UN", moneda: "COP" }));
  ...
  it("IDOR: detalle de un costoId de OTRO tenant ⇒ notFound", async () => {
    const h = must(await exec(ctx(T_A), `${MODULO}.hecho.materializar-material`, { opId: `idor-${RUN}`, otId: "ot1", articuloId: "art1", cantidad: "1", unidad: "UN", moneda: "COP" })) as Record<string, unknown>;
  ...
  it("monedas mixtas para el mismo activo ⇒ series SEPARADAS (nunca sumadas)", async () => {
    ...
    must(await exec(ctx(T_A), `${MODULO}.hecho.materializar-material`, { opId: `usd-${RUN}`, otId: "ot1", articuloId: "artU", cantidad: "1", unidad: "UN", moneda: "USD" }));
