/**
 * DGP-012 · Módulo Enterprise Maintenance Plans — Adaptador REAL de `WorkflowPort`
 ...
 * auto-aprobación): todo plan SOLO transiciona si el motor lo autoriza.
 *
 ...
 * UoW del comando del módulo — mismo patrón que module-inventario (DGP-011.2).
 */
import {
  childContext,
...
/**
 * Definición NEUTRA del ciclo de vida gobernado del plan. El motor exige nombres
 * de estado camelCase; el dominio (plan.ts) usa las MISMAS claves en minúscula
 * (borrador/vigente/suspendido/finalizado/archivado) — no requieren traducción.
 *
 * CICLO DE VIDA: desde `borrador` se `publicar` → `vigente`. Desde `vigente` se
 * puede `suspender`/`posponer` → `suspendido`, `cancelar` → `finalizado`, o
 * `archivar` → `archivado` (terminal). Desde `suspendido` se puede `reanudar`/
 * `extender`/`reprogramar` → `vigente`, o `cancelar` → `finalizado`. Desde
 * `finalizado` sólo `archivar` → `archivado`. Extensible por tenant vía config.
 */
const DEFINICIONES: Record<ProcesoWorkflow, DefinicionWorkflow> = {
  plan: {
    clave: "ciclo-plan-mantenimiento",
    etiqueta: "Plan de mantenimiento",
    // El ciclo declara EXPLÍCITAMENTE sus comandos (suspender/reanudar/cancelar);
    // desactivamos las operaciones estándar del motor para evitar transiciones
    // ambiguas (mismo `de::comando`) contra los estados canónicos del motor.
    ...
    transiciones: [
      { de: "borrador", a: "vigente", comando: "publicar" },
      { de: "borrador", a: "archivado", comando: "archivar" },
      { de: "vigente", a: "suspendido", comando: "suspender" },
      { de: "vigente", a: "suspendido", comando: "posponer" },
      { de: "vigente", a: "finalizado", comando: "cancelar" },
      { de: "vigente", a: "archivado", comando: "archivar" },
      { de: "suspendido", a: "vigente", comando: "reanudar" },
      { de: "suspendido", a: "vigente", comando: "extender" },
      { de: "suspendido", a: "vigente", comando: "reprogramar" },
      { de: "suspendido", a: "finalizado", comando: "cancelar" },
      { de: "finalizado", a: "archivado", comando: "archivar" },
    ],
  },
};
