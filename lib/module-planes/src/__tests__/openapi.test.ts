/**
 * DGP-012.2 · Contract-first — el JSON OpenAPI comprometido debe estar
...
const rutaJson = resolve(aqui, "../../openapi/planes.openapi.json");
...
describe("DGP-012.2 · OpenAPI contract-first", () => {
  ...
  it("cubre CADA comando y consulta literal del módulo (operationId por ruta)", () => {
    ...
    const faltantes = nombres.filter((suf) => !operationIds.has(`planes.${suf}`));
