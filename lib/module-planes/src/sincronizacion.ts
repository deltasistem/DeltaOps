/**
 * DGP-012.2 · Módulo Enterprise Maintenance Plans — Sincronización offline por
 * ORQUESTACIÓN con protocolo de RECLAMACIÓN DURABLE (claim → ejecutar →
 * finalize / release). Réplica exacta del patrón de module-inventario (011.2).
 *
...
const COMANDOS_CREACION = new Set(["crear-plan", "crear-calendario"]);
...
async function estadoActual(runtime: PlatformRuntime, ctx: ExecutionContext, input: Record<string, unknown>): Promise<unknown> {
  const id = typeof input["id"] === "string" ? input["id"] : (typeof input["planId"] === "string" ? input["planId"] : null);
  if (!id) return null;
  const r = await runtime.kernel.queries.execute(ctx, `${MODULO}.plan`, { id });
