/**
 * DGP-015 · Módulo Enterprise Corrective Maintenance — Sincronización offline por
 * ORQUESTACIÓN con protocolo de RECLAMACIÓN DURABLE (claim → ejecutar →
 * finalize / release). Réplica exacta del patrón de module-preventivo (DGP-014).
 *
...
const COMANDOS_CREACION = new Set([
  "crear-solicitud",
...
const QUERY_POR_COMANDO: Record<string, string> = {
  "crear-solicitud": "solicitud-detalle",
  "editar-solicitud": "solicitud-detalle",
  "adjuntar-evidencia": "solicitud-detalle",
  "comentar-solicitud": "solicitud-detalle",
  "transicionar-solicitud": "solicitud-detalle",
  "crear-intervencion": "intervencion-detalle",
  "asignar-cuadrillas": "intervencion-detalle",
  "transicionar-intervencion": "intervencion-detalle",
};
...
async function estadoActual(runtime: PlatformRuntime, ctx: ExecutionContext, comando: string, input: Record<string, unknown>): Promise<unknown> {
  ...
  const id =
    ...
    : typeof input["solicitudId"] === "string" ? input["solicitudId"]
    : typeof input["intervencionId"] === "string" ? input["intervencionId"]
    : null;
  if (!id) return null;
