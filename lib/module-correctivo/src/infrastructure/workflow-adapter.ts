/**
 * DGP-015 · Módulo Enterprise Corrective Maintenance — Adaptador REAL de
 * `WorkflowPort` sobre el Workflow Engine (DGP-007).
 *
 ...
 * auto-aprobación): toda solicitud/intervención/generación SOLO transiciona si el
 * motor lo autoriza.
 *
 ...
 * UoW del comando del módulo — mismo patrón que module-preventivo (DGP-014).
 *
 * El motor multiplexa por `clave` bajo un único proceso `modulo.correctivo.workflow`:
 * las tres definiciones (solicitud/intervencion/generacion) conviven sin colisión.
 */
import {
  childContext,
 ...
 * exige estados camelCase/neutros; el dominio (solicitud.ts / intervencion.ts /
 * orden-correctiva.ts) usa las MISMAS claves neutras — no requieren traducción.
 *
 *  · solicitud    : registro →(enviarTriage)→ triage →(iniciarDiagnostico)→
 *                   diagnostico →(enviarValidacion)→ validacion →(aprobar)→
 *                   aprobada[final]; triage/diagnostico/validacion →(rechazar)→
 *                   rechazada[final].
 *  · intervencion : preparacion →(asignar)→ asignacion →(iniciarEjecucion)→
 *                   ejecucion →(enviarVerificacion)→ verificacion →(cerrar)→
 *                   cerrada[final].
 *  · generacion   : pendiente →(materializar)→ materializada[final].
 */
const DEFINICIONES: Record<ProcesoWorkflow, DefinicionWorkflow> = {
  solicitud: {
    clave: "cor-atencion",
    etiqueta: "Solicitud correctiva",
    operacionesEstandar: { cancelar: false, reabrir: false, suspender: false, reanudar: false },
    estados: [
      { nombre: "registro", inicial: true },
      { nombre: "triage" },
      { nombre: "diagnostico" },
      { nombre: "validacion" },
    ...
    transiciones: [
      { de: "registro", a: "triage", comando: "enviarTriage" },
      { de: "triage", a: "diagnostico", comando: "iniciarDiagnostico" },
      { de: "diagnostico", a: "validacion", comando: "enviarValidacion" },
      { de: "validacion", a: "aprobada", comando: "aprobar" },
      { de: "triage", a: "rechazada", comando: "rechazar" },
      { de: "diagnostico", a: "rechazada", comando: "rechazar" },
      { de: "validacion", a: "rechazada", comando: "rechazar" },
    ],
  },
  intervencion: {
    clave: "cor-intervencion",
    etiqueta: "Intervención correctiva",
    operacionesEstandar: { cancelar: false, reabrir: false, suspender: false, reanudar: false },
    estados: [
      { nombre: "preparacion", inicial: true },
      { nombre: "asignacion" },
      { nombre: "ejecucion" },
      { nombre: "verificacion" },
    ...
    transiciones: [
      { de: "preparacion", a: "asignacion", comando: "asignar" },
      { de: "asignacion", a: "ejecucion", comando: "iniciarEjecucion" },
      { de: "ejecucion", a: "verificacion", comando: "enviarVerificacion" },
      { de: "verificacion", a: "cerrada", comando: "cerrar" },
    ],
  },
  generacion: {
    clave: "cor-generacion",
    etiqueta: "Generación correctiva",
    operacionesEstandar: { cancelar: false, reabrir: false, suspender: false, reanudar: false },
    estados: [
      { nombre: "pendiente", inicial: true },
      { nombre: "materializada", final: true },
    ],
    transiciones: [
      { de: "pendiente", a: "materializada", comando: "materializar" },
    ],
  },
};
