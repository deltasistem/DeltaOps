/**
 * DGP-014 · Módulo Enterprise Preventive Maintenance — Adaptador REAL de
 ...
 * auto-aprobación): todo programa/generación SOLO transiciona si el motor lo
 * autoriza.
 *
 ...
 * UoW del comando del módulo — mismo patrón que module-abastecimiento (DGP-013.2).
 */
import {
  childContext,
 ...
 * exige estados camelCase; el dominio (programa.ts / generacion.ts) usa las
 * MISMAS claves neutras — no requieren traducción.
 *
 *  · programa    : preparacion →(enviarRevision)→ revision →(publicar)→ publicado;
 *                  publicado →(suspender)→ suspendido →(reanudar)→ publicado;
 *                  publicado/suspendido →(archivar)→ archivado[final].
 *  · generacion  : pendiente →(materializar)→ materializada[final].
 */
const DEFINICIONES: Record<ProcesoWorkflow, DefinicionWorkflow> = {
  programa: {
    clave: "ciclo-programa",
    etiqueta: "Programa preventivo",
    ...
    transiciones: [
      { de: "preparacion", a: "revision", comando: "enviarRevision" },
      { de: "revision", a: "publicado", comando: "publicar" },
      { de: "publicado", a: "suspendido", comando: "suspender" },
      { de: "suspendido", a: "publicado", comando: "reanudar" },
      { de: "publicado", a: "archivado", comando: "archivar" },
      { de: "suspendido", a: "archivado", comando: "archivar" },
    ],
  },
  generacion: {
    clave: "ciclo-generacion",
    etiqueta: "Generación preventiva",
    operacionesEstandar: { cancelar: false, reabrir: false, suspender: false, reanudar: false },
    estados: [
