/**
 * DGP-014.2 · Contract-first — el JSON OpenAPI comprometido debe estar
...
const rutaJson = resolve(aqui, "../../openapi/preventivo.openapi.json");
...
describe("DGP-014.2 · OpenAPI contract-first", () => {
  ...
  it("cubre CADA comando y consulta literal del módulo (operationId por ruta)", () => {
    ...
    const faltantes = nombres.filter((suf) => !operationIds.has(`preventivo.${suf}`));
