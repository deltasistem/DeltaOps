/**
 * DGP-014 · Módulo Enterprise Preventive Maintenance — Sincronización offline por
 * ORQUESTACIÓN con protocolo de RECLAMACIÓN DURABLE (claim → ejecutar →
 * finalize / release). Réplica exacta del patrón de module-abastecimiento (013.2).
 *
...
const QUERY_POR_COMANDO: Record<string, string> = {
  "crear-programa": "programa",
  "editar-programa": "programa",
  "transicionar-programa": "programa",
  "versionar-programa": "programa",
  "revertir-programa": "programa",
};
...
async function estadoActual(runtime: PlatformRuntime, ctx: ExecutionContext, comando: string, input: Record<string, unknown>): Promise<unknown> {
  ...
  const id =
    ...
    : typeof input["programaId"] === "string" ? input["programaId"]
    : null;
  if (!id) return null;
