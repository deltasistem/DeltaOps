/**
 * DGP-008.2 · Contract-first — el JSON OpenAPI comprometido debe estar
 ...
 * aparece como operación del contrato).
 */
...
const rutaJson = resolve(aqui, "../../openapi/activos.openapi.json");
...
describe("DGP-008.2 · OpenAPI contract-first", () => {
  ...
  it("el documento es OpenAPI 3 con rutas y esquemas", () => {
    const doc = construirOpenApi() as { openapi: string; paths: Record<string, unknown>; components: { schemas: Record<string, unknown> } };
  ...
  it("cubre CADA comando y consulta del módulo (operationId por ruta)", () => {
    ...
    // Cada comando/consulta del módulo debe tener una operación `activos.<sufijo>`
    // (los sufijos con punto, p.ej. catalogo.opciones, se mapean 1:1).
    const faltantes = nombres.filter((suf) => !operationIds.has(`activos.${suf}`));
