/**
 * DGP-013.2 · Módulo Enterprise Procurement — Sincronización offline por
 * ORQUESTACIÓN con protocolo de RECLAMACIÓN DURABLE (claim → ejecutar →
 * finalize / release). Réplica exacta del patrón de module-planes (012.2) /
 * module-inventario (011.2).
 *
...
const QUERY_POR_COMANDO: Record<string, string> = {
  ...
  "calificar-proveedor": "proveedor",
  "crear-solicitud": "solicitud",
  "transicionar-solicitud": "solicitud",
  "crear-orden-compra": "orden-compra",
  "transicionar-orden-compra": "orden-compra",
};
...
async function estadoActual(runtime: PlatformRuntime, ctx: ExecutionContext, comando: string, input: Record<string, unknown>): Promise<unknown> {
  ...
  const id =
    ...
    : typeof input["ordenCompraId"] === "string" ? input["ordenCompraId"]
    : typeof input["solicitudId"] === "string" ? input["solicitudId"]
    : typeof input["proveedorId"] === "string" ? input["proveedorId"]
    : null;
  if (!id) return null;
