/**
 * DGP-019.1 · Contract-first — el JSON OpenAPI comprometido debe estar
 * SINCRONIZADO con el generador determinista (regenerar == comprometido) y DEBE
 * cubrir TODOS los comandos/consultas del módulo (cada comando/consulta con
 * nombre literal aparece como operación del contrato con operationId
 * `utilizacion.<sufijo>`).
 */
...
const rutaJson = resolve(aqui, "../../openapi/utilizacion.openapi.json");
...
function nombresDelModulo(): string[] {
  ...
  // El eventType `${MODULO}.sincronizar-activo` NO es comando ni consulta HTTP.
  out.delete("sincronizar-activo");
  return [...out];
}

describe("DGP-019.1 · OpenAPI contract-first", () => {
  ...
  it("el documento es OpenAPI 3 con rutas y esquemas", () => {
    ...
    expect(Object.keys(doc.paths).length).toBeGreaterThanOrEqual(12);
  ...
  it("cubre CADA comando y consulta literal del módulo (operationId por ruta)", () => {
    ...
    const faltantes = nombres.filter((suf) => !operationIds.has(`utilizacion.${suf}`));
