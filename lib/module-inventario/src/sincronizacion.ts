/**
 * DGP-011.2 · Módulo Enterprise Inventory — Sincronización offline por
 * ORQUESTACIÓN con protocolo de RECLAMACIÓN DURABLE (claim → ejecutar →
 * finalize / release). Réplica exacta del patrón de `module-ordenes` (DGP-009.2).
 *
...
const COMANDOS_CREACION = new Set([
  "crear-item", "crear-bodega", "crear-ubicacion", "crear-lote",
  "registrar-serie", "reservar", "transferir", "iniciar-conteo",
]);
...
async function estadoActual(
  ...
  if (!id) return null;
  const r = await runtime.kernel.queries.execute(ctx, `${MODULO}.item`, { id });
