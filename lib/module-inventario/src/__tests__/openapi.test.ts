/**
 * DGP-011.2 · Contract-first — el JSON OpenAPI comprometido debe estar
 * SINCRONIZADO con el generador determinista (regenerar == comprometido) y
 * DEBE cubrir TODAS las rutas literales del módulo (cada comando/consulta
 * HTTP-expuesto con nombre literal aparece como operación del contrato).
 */
...
const rutaJson = resolve(aqui, "../../openapi/inventario.openapi.json");
...
describe("DGP-011.2 · OpenAPI contract-first", () => {
  ...
  it("cubre CADA comando y consulta literal del módulo (operationId por ruta)", () => {
    ...
    const faltantes = nombres.filter((suf) => !operationIds.has(`inventario.${suf}`));
