/**
 * DGP-016 · Contract-first — el JSON OpenAPI comprometido debe estar
...
const rutaJson = resolve(aqui, "../../openapi/analytics.openapi.json");
...
describe("DGP-016 · OpenAPI contract-first (Analytics)", () => {
  ...
  it("el documento es OpenAPI 3 con rutas y esquemas", () => {
    ...
    expect(Object.keys(doc.components.schemas).length).toBeGreaterThan(15);
  });
  ...
  it("cubre CADA comando y consulta literal del módulo (operationId por ruta)", () => {
    ...
    const faltantes = nombres.filter((suf) => !operationIds.has(`analytics.${suf}`));
