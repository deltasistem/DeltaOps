/**
 * DGP-016 · Módulo Enterprise Analytics & KPI Platform — Sincronización offline por
 * ORQUESTACIÓN con protocolo de RECLAMACIÓN DURABLE (claim → ejecutar →
 * finalize / release). Réplica exacta del patrón de module-correctivo (DGP-015).
 *
 * `procesarCola` NO es un comando del Kernel que envuelva a otros (eso anidaría
 * UoWs): orquesta cada operación offline con su PROPIA UoW (una por comando).
...
/**
 * Comandos de CREACIÓN offline que EXIGEN `id` de cliente (Offline First). Los
 * snapshots son idempotentes por clave determinista (no exigen id); las
 * definiciones/dashboards personalizados creados offline SÍ exigen id de cliente.
 */
const COMANDOS_CREACION = new Set<string>([]);
...
const QUERY_POR_COMANDO: Record<string, string> = {
  ...
  "habilitar-indicador": "indicador",
  "actualizar-dashboard": "dashboard",
  "clonar-dashboard": "dashboard",
  "eliminar-dashboard": "dashboard",
};
...
async function estadoActual(runtime: PlatformRuntime, ctx: ExecutionContext, comando: string, input: Record<string, unknown>): Promise<unknown> {
  ...
  if (query === "indicador") {
    const clave = typeof input["clave"] === "string" ? input["clave"] : null;
    if (!clave) return null;
    const r = await runtime.kernel.queries.execute(ctx, `${MODULO}.indicador`, { clave });
    return r.ok ? r.value : null;
  }
  const id = typeof input["id"] === "string" ? input["id"] : null;
  const clave = typeof input["clave"] === "string" ? input["clave"] : null;
  if (!id && !clave) return null;
  const r = await runtime.kernel.queries.execute(ctx, `${MODULO}.dashboard`, { ...(id ? { id } : {}), ...(clave ? { clave } : {}) });
  return r.ok ? r.value : null;
}
...
export async function procesarCola(
  ...
  if (!receipts) {
    for (const op of operaciones) {
      resultados.push({ opId: op.opId, comando: nombreComando(op.comando), estado: "rechazada", error: "El runtime no está configurado con recibos de sincronización durables." });
    }
    return resumir(resultados);
  }
  ...
  for (const op of operaciones) {
    ...
    if (reintentable) {
      ...
      await receipts.finalize(tenantId, { opId: op.opId, clienteId, comando, estado: resultado.estado, resultado });
    }
