/**
 * DGP-009.2 · Contract-first — el JSON OpenAPI comprometido debe estar
...
const rutaJson = resolve(aqui, "../../openapi/ordenes.openapi.json");
...
describe("DGP-009.2 · OpenAPI contract-first", () => {
  ...
  it("cubre CADA comando y consulta del módulo (operationId por ruta)", () => {
    ...
    // Cada comando/consulta del módulo debe tener una operación `ordenes.<sufijo>`.
    const faltantes = nombres.filter((suf) => !operationIds.has(`ordenes.${suf}`));
