/**
 * DGP-012 · Cliente HTTP del módulo Enterprise Maintenance Plans.
 *
 ...
 * Idéntico patrón al cliente de Inventario (DGP-011.3), apuntando a /planes.
 */
...
export class PlanesApiError extends Error {
  readonly status: number;
  readonly code?: string;
  constructor(e: ApiError) {
    super(e.error);
    this.name = "PlanesApiError";
...
/**
 * Ejecuta una petición al módulo. Lanza `PlanesApiError` en fallo salvo que
 * `toleraNoEncontrado` esté activo y el estado sea 404 (devuelve `null`).
 * Redirige a /login en 401.
 */
export async function planesFetch<T = unknown>(path: string, opts: FetchOpts = {}): Promise<T> {
...
export function esFuncionNoDisponible(err: unknown): boolean {
  return err instanceof PlanesApiError && err.status === 404;
}
