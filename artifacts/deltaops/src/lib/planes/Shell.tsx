/**
 * DGP-012 · Shell de la experiencia Enterprise Maintenance Plans.
 *
 ...
 * (OfflineProvider con espacio de nombres "planes" + banner de estado).
 * Réplica exacta del patrón probado del Shell de Inventario (DGP-011.3).
 */
...
const NAV: { href: string; etiqueta: string }[] = [
  ...
  { href: "/planes/nuevo", etiqueta: "Nuevo plan" },
  { href: "/planes/calendario", etiqueta: "Calendario" },
  { href: "/planes/sincronizacion", etiqueta: "Sincronización" },
];
...
export interface ShellPlanesProps {
  children: React.ReactNode;
  /** Ruta activa para resaltar en la navegación. */
  activo?: string;
}

export function ShellPlanes({ children, activo }: ShellPlanesProps) {
      ...
      <OfflineProvider tenant={TENANT} modulo={MODULO_OFFLINE} syncUrl={SYNC_URL}>
        <div className="do-root" style={{ minHeight: "100vh", background: "var(--do-bg)" }}>
          ...
          >
            ...
            <strong style={{ marginRight: "var(--do-sp-2)" }}>Planes de mantenimiento</strong>
            {NAV.map((n) => {
              const esActivo = rutaActiva === n.href || (n.href !== "/planes" && rutaActiva.startsWith(n.href + "/"));
              return (
