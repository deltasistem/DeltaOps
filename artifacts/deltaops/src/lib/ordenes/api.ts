/**
 * DGP-009.3 · Cliente HTTP del módulo de Órdenes de Trabajo.
 *
 ...
 * Idéntico patrón al cliente de Activos (DGP-008.3), apuntando a /ordenes.
 */
...
/**
 * Ejecuta una petición al módulo. Lanza `OrdenesApiError` en fallo salvo que
 * `toleraNoEncontrado` esté activo y el estado sea 404 (devuelve `null`).
 * Redirige a /login en 401.
 */
export async function ordenesFetch<T = unknown>(path: string, opts: FetchOpts = {}): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    throw new OrdenesApiError({ status: 401, error: "No autenticado" });
  }
...
export function esFuncionNoDisponible(err: unknown): boolean {
  return err instanceof OrdenesApiError && err.status === 404;
}
