/**
 * DGP-011.3 · Shell de la experiencia Enterprise Inventory.
 *
 ...
 * (OfflineProvider con espacio de nombres "inventario" + banner de estado).
 * Réplica exacta del patrón probado del Shell de Órdenes (DGP-009.3).
 */
...
const NAV: { href: string; etiqueta: string }[] = [
  ...
  { href: "/inventario/transferencias", etiqueta: "Transferencias" },
  { href: "/inventario/conteos", etiqueta: "Conteos" },
  { href: "/inventario/bodegas", etiqueta: "Bodegas" },
  { href: "/inventario/nuevo", etiqueta: "Nuevo" },
  { href: "/inventario/escanear", etiqueta: "Escanear" },
  { href: "/inventario/sincronizacion", etiqueta: "Sincronización" },
];
...
export interface ShellInventarioProps {
  children: React.ReactNode;
  /** Ruta activa para resaltar en la navegación. */
  activo?: string;
}

export function ShellInventario({ children, activo }: ShellInventarioProps) {
      ...
      <OfflineProvider tenant={TENANT} modulo={MODULO_OFFLINE} syncUrl={SYNC_URL}>
        <div className="do-root" data-do-theme="light" style={{ minHeight: "100vh", background: "var(--do-bg)" }}>
          ...
          >
            ...
            <strong style={{ marginRight: "var(--do-sp-2)" }}>Inventario</strong>
            {NAV.map((n) => {
              const esActivo = rutaActiva === n.href || (n.href !== "/inventario" && rutaActiva.startsWith(n.href + "/"));
              return (
