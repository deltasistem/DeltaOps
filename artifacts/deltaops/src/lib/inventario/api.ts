/**
 * DGP-011.3 · Cliente HTTP del módulo Enterprise Inventory.
 *
 ...
 * Idéntico patrón al cliente de Órdenes (DGP-009.3), apuntando a /inventario.
 */
...
/**
 * Ejecuta una petición al módulo. Lanza `InventarioApiError` en fallo salvo que
 * `toleraNoEncontrado` esté activo y el estado sea 404 (devuelve `null`).
 * Redirige a /login en 401.
 */
export async function inventarioFetch<T = unknown>(path: string, opts: FetchOpts = {}): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    throw new InventarioApiError({ status: 401, error: "No autenticado" });
  }
...
export function esFuncionNoDisponible(err: unknown): boolean {
  return err instanceof InventarioApiError && err.status === 404;
}
