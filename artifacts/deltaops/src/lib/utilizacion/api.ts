/**
 * DGP-019.1 · Cliente HTTP del módulo Utilización, Medidores y Combustible.
 *
 ...
 * Idéntico patrón al cliente de Correctivo/Preventivo, apuntando a /utilizacion.
 */
...
export class UtilizacionApiError extends Error {
  readonly status: number;
  readonly code?: string;
  constructor(e: ApiError) {
    super(e.error);
    this.name = "UtilizacionApiError";
...
/**
 * Ejecuta una petición al módulo. Lanza `UtilizacionApiError` en fallo salvo que
 * `toleraNoEncontrado` esté activo y el estado sea 404 (devuelve `null`).
 * Redirige a /login en 401.
 */
export async function utilizacionFetch<T = unknown>(path: string, opts: FetchOpts = {}): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    throw new UtilizacionApiError({ status: 401, error: "No autenticado" });
  }
...
export function esFuncionNoDisponible(err: unknown): boolean {
  return err instanceof UtilizacionApiError && err.status === 404;
}
