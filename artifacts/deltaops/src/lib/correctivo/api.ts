/**
 * DGP-015 · Cliente HTTP del módulo Enterprise Corrective Maintenance.
 *
 ...
 * Idéntico patrón al cliente de Preventivo/Planes, apuntando a /correctivo.
 */
...
export class CorrectivoApiError extends Error {
  readonly status: number;
  readonly code?: string;
  constructor(e: ApiError) {
    super(e.error);
    this.name = "CorrectivoApiError";
...
/**
 * Ejecuta una petición al módulo. Lanza `CorrectivoApiError` en fallo salvo que
 * `toleraNoEncontrado` esté activo y el estado sea 404 (devuelve `null`).
 * Redirige a /login en 401.
 */
export async function correctivoFetch<T = unknown>(path: string, opts: FetchOpts = {}): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    throw new CorrectivoApiError({ status: 401, error: "No autenticado" });
  }
...
export function esFuncionNoDisponible(err: unknown): boolean {
  return err instanceof CorrectivoApiError && err.status === 404;
}
