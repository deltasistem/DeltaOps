/**
 * DGP-015 · Shell de la experiencia Enterprise Corrective Maintenance.
 *
 ...
 * (OfflineProvider con espacio de nombres "correctivo" + banner de estado).
 * Réplica exacta del patrón probado del Shell de Preventivo (DGP-014).
 */
...
const NAV: { href: string; etiqueta: string }[] = [
  { href: "/correctivo/solicitudes", etiqueta: "Solicitudes" },
  { href: "/correctivo/solicitudes/nueva", etiqueta: "Nueva solicitud" },
  { href: "/correctivo/escanear", etiqueta: "Escanear" },
  { href: "/correctivo/sincronizacion", etiqueta: "Sincronización" },
];
...
export interface ShellCorrectivoProps {
  children: React.ReactNode;
  /** Ruta activa para resaltar en la navegación. */
  activo?: string;
}

export function ShellCorrectivo({ children, activo }: ShellCorrectivoProps) {
      ...
      <OfflineProvider tenant={TENANT} modulo={MODULO_OFFLINE} syncUrl={SYNC_URL}>
        <div className="do-root" style={{ minHeight: "100vh", background: "var(--do-bg)" }}>
          ...
          >
            ...
            <strong style={{ marginRight: "var(--do-sp-2)" }}>Mantenimiento correctivo</strong>
            {NAV.map((n) => {
              const base = "/correctivo/solicitudes";
              const esBase = n.href === base;
              const esActivo = esBase
                ? rutaActiva === base || (rutaActiva.startsWith(base + "/") && !rutaActiva.startsWith(base + "/nueva"))
