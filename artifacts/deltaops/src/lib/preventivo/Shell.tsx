/**
 * DGP-014 · Shell de la experiencia Enterprise Preventive Maintenance.
 *
 ...
 * (OfflineProvider con espacio de nombres "preventivo" + banner de estado).
...
const NAV: { href: string; etiqueta: string }[] = [
  { href: "/preventivo/programas", etiqueta: "Programas" },
  { href: "/preventivo/programas/nuevo", etiqueta: "Nuevo programa" },
  { href: "/preventivo/calendario", etiqueta: "Calendario" },
  { href: "/preventivo/escanear", etiqueta: "Escanear" },
  { href: "/preventivo/sincronizacion", etiqueta: "Sincronización" },
];
...
export interface ShellPreventivoProps {
  children: React.ReactNode;
  /** Ruta activa para resaltar en la navegación. */
  activo?: string;
}

export function ShellPreventivo({ children, activo }: ShellPreventivoProps) {
    ...
    <ThemeProvider>
      <OfflineProvider tenant={TENANT} modulo={MODULO_OFFLINE} syncUrl={SYNC_URL}>
        <div className="do-root" data-do-theme="light" style={{ minHeight: "100vh", background: "var(--do-bg)" }}>
          <BannerOffline />
          <nav
            aria-label="Navegación de preventivo"
            style={{
              display: "flex",
              alignItems: "center",
              gap: "var(--do-sp-2)",
          ...
          >
            ...
            <strong style={{ marginRight: "var(--do-sp-2)" }}>Mantenimiento preventivo</strong>
            {NAV.map((n) => {
              const base = "/preventivo/programas";
              const esProgramasBase = n.href === base;
              const esActivo = esProgramasBase
                ? rutaActiva === base || (rutaActiva.startsWith(base + "/") && !rutaActiva.startsWith(base + "/nuevo"))
