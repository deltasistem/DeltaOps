/**
 * DGP-015 · Escaneo QR → mantenimiento correctivo de un activo.
 *
 * El correctivo NO define un QR propio: se ancla al QR del ACTIVO (platform.qr,
 * patrón DGP-014). Al escanear (cámara BarcodeDetector con reserva de entrada
 * manual) se resuelve el código con el resolvedor del servidor (fuente primaria)
 * y, sólo como degradación, interpretación LOCAL del contenido. El destino es la
 * pestaña «Correctivo» de la ficha del activo (historial de eventos/
 * reincidencias) y el atajo para levantar una nueva solicitud anclada al activo.
 */
...
import { urlActivo, urlNuevaSolicitud } from "../lib/correctivo/deep-links";

export default function CorrectivoEscanearPage() {
  return (
    <ShellCorrectivo activo="/correctivo/escanear">
      <Escanear />
    </ShellCorrectivo>
  );
}
  ...
  return (
    <>
      <PageHeader
        ...
        descripcion="Escanea el QR del activo para ver su historial correctivo (eventos y reincidencias) o levantar una nueva solicitud."
        acciones={
      ...
      {activoId ? (
        <Section titulo="Activo resuelto" acciones={<Button variant="fantasma" size="sm" onClick={() => { setActivoId(null); form.setValores({}); }}>Escanear otro</Button>}>
          <Card><CardContent>
            <p>Activo <strong>{activoId}</strong> resuelto. Continúa con su mantenimiento correctivo:</p>
            <div style={{ display: "flex", gap: "var(--do-sp-2)", flexWrap: "wrap" }}>
              <Link href={`${urlActivo(activoId)}?tab=correctivo`}><Button variant="primario">Ver historial correctivo</Button></Link>
              <Link href={urlNuevaSolicitud({ activo: activoId })}><Button variant="secundario">Nueva solicitud</Button></Link>
              <Link href={urlActivo(activoId)}><Button variant="fantasma">Ficha del activo</Button></Link>
            </div>
          </CardContent></Card>
        </Section>
      ) : (
