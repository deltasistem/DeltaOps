...
- **Política de inconsistencia (documentada antes de implementar, alineada con la política canónica de Activos):** Activos, por defecto, **rechaza** retrocesos de medidor y solo los permite con configuración explícita; el módulo nuevo adopta esa política como autoridad del "último valor". Si `valor < última lectura válida del mismo medidor`: la lectura se conserva en el historial con `inconsistente=true` y motivo, se emite `modulo.utilizacion.lectura-inconsistente` y **NO se propaga** a Activos. No hay dos semánticas: el historial refleja siempre lo capturado (auditabilidad), el último valor refleja solo lo que la política de Activos acepta.
- **Regularización `reinicioMedidor` (gobernada):** declara cambio/reinicio de medidor físico y ancla un nuevo tramo de deltas. Requiere capacidad dedicada `utilizacion.medidores.regularizar` (SUPERVISOR/TENANT_ADMIN; TECNICO no), queda en timeline, y su propagación a Activos usa exclusivamente el mecanismo explícito de retroceso que Activos ya contempla por configuración — jamás un bypass.
- **Validación de tipo/unidad:** `horometro` (h) y `odometro/kilometraje` (km) son tipos canónicos inmutables; el catálogo de medidores personalizados no puede redefinir ni colisionar con las claves canónicas ni sus unidades (validación de dominio + test). Solo los canónicos se propagan a Activos (GAP-F).
- **Protocolo de consistencia con Activos (resuelve expectedVersion y atomicidad):** el comando `registrar-lectura` persiste la lectura y encola la propagación **en la misma UoW** (outbox durable propio del flujo, handler idempotente at-least-once — disciplina kernel existente). La lectura nace con estado `sincronizacionActivo: pendiente | confirmada | no-aplica | fallida`, expuesto en queries. El handler de propagación, serializado por (tenant, activo, medidor): (1) relee `detalle` público de Activos para obtener la **versión vigente** y el último valor; (2) si existe una lectura válida más reciente (`ocurridoAt`) ya propagada o en cola, marca la actual `no-aplica` (gana siempre la más reciente; un reintento tardío nunca deja el último valor obsoleto); (3) invoca `actualizar-horometro/odometro` con la `expectedVersion` releída; (4) ante 409 por concurrencia, relee y reintenta (bounded); agotados los reintentos marca `fallida` con motivo visible y evento — nunca divergencia silenciosa. La lectura histórica es válida aunque la propagación esté pendiente/fallida: la fuente de verdad del historial es el módulo; la del último valor, Activos.
...
- Tenancy/RLS: `tenantOf(ctx)` fail-closed en cada handler, `set_config` por transacción, mínimo privilegio por rol (TECNICO captura; CONSULTA solo lee; capacidades nuevas `utilizacion.lecturas.registrar`, `utilizacion.tanqueos.registrar`, `utilizacion.medidores.regularizar`, `utilizacion.leer` incorporadas al catálogo canónico de autorización backend — resolución capability→permisos del kernel — jamás solo mapeadas en frontend). Autorización solo desde la sesión (DGP-017). Nada de filtros solo-frontend.
...
- **R3 — Doble verdad del último medidor**: mitigado por el protocolo de consistencia de §4.1 (outbox durable en la misma UoW, relectura de versión, serialización por medidor, "gana la más reciente", estado de sincronización visible y fallo ruidoso); el historial nunca vive en Activos ni el último valor en Utilización.
